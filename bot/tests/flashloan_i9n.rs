...
use defi_arbitrage_bot::objectiveFunc::optimize_for_flash_loan;
use defi_arbitrage_bot::{cronJob::cron_job, objectiveFunc};
use ethers::{
...
#[tokio::test]
async fn test_loop() {
    let (endpoint, flash_loan_address, anvil, wallet_with_chain_id, client_address, weth_contract, _) =
        setup().await.unwrap();
    ...
    let tx_receipt = cron_job(endpoint, true, flash_loan_address, wallet_with_chain_id).await.unwrap();
...
async fn test_find_optimize_val() {
    let (endpoint, flash_loan_address, anvil, wallet_with_chain_id, client_address, weth_contract, flash_loan_contract) =
        setup().await.unwrap();
    ...
    loop {
        let data = ethers::abi::encode(&[
            ...
            ethers::abi::Token::Address("0x6B175474E89094C44Da98b954EedeAC495271d0F".parse::<Address>().unwrap()),
        ]);
        let flash_call = flash_loan_contract.flash_loan(U256::from(0), lower_bound, data.into());
    ...
    loop {
        ...
        let flash_call = flash_loan_contract.flash_loan(U256::from(0), upper_bound, data.into());
        match flash_call.call().await {
            Ok(weth_balance_increase) => break,
            Err(e) => {
    ...
    let optimal_val = objectiveFunc::golden_section_search(
        lower_bound,
        upper_bound,
        optimize_for_flash_loan,
        U256::from(100),
        endpoint,
        flash_loan_address,
